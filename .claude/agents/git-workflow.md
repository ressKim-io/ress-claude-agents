...
- 로컬 `git add` / `git commit` 은 수행할 수 있으나, 원격에 영향을 주는
  `git push` / `gh pr create` / `gh pr comment` / `gh release create` 는
  **직접 실행하지 않는다** — 메인 에이전트가 사용자 승인을 받은 뒤 실행한다.
- 본 문서의 `gh` / `git push` 명령 블록은 전부 "메인 에이전트가 승인 후 실행할
  참고 명령"이다. agent 자신이 실행하는 절차가 아니다.

## Escalation (중단·이관 기준)

다음 중 하나면 작업을 멈추고 메인 에이전트에 결과 + 사유를 반환한다:
- 원격 상태 변경(push / PR / release)이 필요한 단계 — 초안만 반환
- 커밋할 변경의 의도가 불명확하거나 staged 내용이 비어 있음
- 브랜치 전략·컨벤션이 레포 설정과 충돌해 단독 판단 불가
반환 형식: `[BLOCKED] <사유> — 제안: <메인 에이전트 또는 사용자 액션>`
