...
|---|---|
| shellcheck | exit 0 |
| `validate-skill-frontmatter.sh agents` | exit 0, `PASS 49/49` + `WARN 144건`(레거시 soft) |
...
### 4. (PR #28 리뷰 후속) 검증 패턴 false negative + set -e 회귀
멀티 관점 리뷰가 CR-001~009를 지적. 핵심 CR-001: 검증 패턴 `^#{2,}[[:space:]].*$2`가
`### Escalation Path` 류 H3 헤더에 오매치 → soft 경고가 144 대신 141로 과소 계상.
`^##[[:space:]]+$2` H2 한정으로 수정. CR-009(VERBOSE 분리) 적용 중 `[[ ]] && cmd`를
`check_agents()` 마지막 명령으로 둬 — 조건 false 시 exit 1 → `set -e` 종료 →
`agents`/`all` 모드 회귀. 명시적 `if`로 재수정. 초기 6단계 검증 "통과"가 이 false
negative를 못 걸렀고, 멀티 관점 리뷰가 잡았다 — 검증과 리뷰는 상호 보완이지 대체가 아니다.
...
- Plan ID: `enchanted-prancing-falcon` (로컬 plan 파일 — 레포 미포함)
