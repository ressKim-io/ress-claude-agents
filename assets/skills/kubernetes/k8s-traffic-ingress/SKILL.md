...
description: "Use when configuring legacy networking.k8s.io Ingress with NGINX or Kong rate limiting (limit-rps, limit-rpm, limit-connections, burst-multiplier, IP whitelist), per-Consumer KongPlugin policies, or Redis-backed distributed rate limiting. Skip when migrating to Gateway API (use gateway-api / gateway-api-migration) or when using L4 Service routing only."
version: 1.0.0
license: MIT
applies_when:
  files_present:
    ...
    - "**/kongconsumer*.yaml"
  files_contain:
    "**/*.yaml": '(nginx\.ingress\.kubernetes\.io/limit|konghq\.com/plugins|kind:\s*KongPlugin|kind:\s*KongConsumer|plugin:\s*rate-limiting|ingressClassName:\s*(nginx|kong))'
  frameworks:
    - kubernetes
    - nginx-ingress
    - kong
