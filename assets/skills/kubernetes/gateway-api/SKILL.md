---
name: gateway-api
description: "Use when defining or reviewing Gateway API resources (GatewayClass, Gateway, HTTPRoute, GRPCRoute, TCPRoute, UDPRoute) for in-cluster L7 routing, mTLS, rate limiting, JWT auth via Envoy Gateway / Kong / Cilium / Istio. Skip when only legacy networking.k8s.io Ingress is in scope (use k8s-traffic-ingress) or when service-mesh East-West routing is the focus."
version: 1.0.0
license: MIT
applies_when:
  files_present:
    ...
    - "**/envoyproxy*.yaml"
  files_contain:
    "**/*.yaml": '(gateway\.networking\.k8s\.io|gateway\.envoyproxy\.io|kind:\s*(Gateway|HTTPRoute|GRPCRoute|TCPRoute|UDPRoute|GatewayClass|EnvoyProxy))'
  frameworks:
    - kubernetes
    - gateway-api
portability:
