...
description: "Use when configuring Karpenter (NodePool, EC2NodeClass, consolidation, Spot+On-Demand mix, disruption budgets) or designing combined autoscaling stack (HPA + KEDA event-driven + VPA right-sizing + Karpenter node provisioning) for production workloads. Skip when only basic HPA/VPA/KEDA is needed (use k8s-autoscaling) or when no node-level autoscaler is in scope."
version: 1.0.0
license: MIT
applies_when:
  files_present:
    ...
    - "**/karpenter*.yaml"
  files_contain:
    "**/*.yaml": '(karpenter\.sh/v1|karpenter\.k8s\.aws|kind:\s*(NodePool|EC2NodeClass)|consolidationPolicy:|disruption:\s*$)'
  frameworks:
    - kubernetes
    - karpenter
portability:
