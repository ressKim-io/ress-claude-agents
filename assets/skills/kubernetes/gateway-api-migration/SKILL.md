...
description: "Use when migrating Ingress NGINX (EOL 2026) to Gateway API: nginx annotation → Policy mapping (rewrite-target, ssl-redirect, rate-limit, auth-tls, cors, proxy-body-size), parallel run with traffic shift, Istio Gateway API mode activation, and TCPRoute/UDPRoute promotion. Skip when greenfield Gateway API only (no legacy Ingress) or when staying on Ingress NGINX with no migration intent."
version: 1.0.0
license: MIT
applies_when:
  files_present:
    ...
    - "**/udproute*.yaml"
  files_contain:
    "**/*.yaml": '(networking\.k8s\.io/v1.*\n.*kind:\s*Ingress|kind:\s*Ingress|gateway\.networking\.k8s\.io|nginx\.ingress\.kubernetes\.io/)'
  frameworks:
    - kubernetes
    - gateway-api
portability:
