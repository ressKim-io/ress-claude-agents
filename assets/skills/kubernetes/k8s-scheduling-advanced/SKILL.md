...
description: "Use for advanced scheduling scenarios: prod/staging workload isolation via taints+tolerations, multi-AZ HA via topologySpreadConstraints (replicas ≥ 6), data locality (pod affinity to colocate cache + app on same node), and scheduling-failure debugging via kubectl describe events. Skip when only basic Deployment placement is needed (use k8s-scheduling)."
version: 1.0.0
license: MIT
applies_when:
  files_present:
    ...
    - "**/statefulset*.yaml"
  files_contain:
    "**/*.yaml": '(topologySpreadConstraints:|whenUnsatisfiable:|maxSkew:|podAntiAffinity:|requiredDuringSchedulingIgnoredDuringExecution:|topology\.kubernetes\.io/zone)'
  frameworks:
    - kubernetes
portability:
  level: universal
